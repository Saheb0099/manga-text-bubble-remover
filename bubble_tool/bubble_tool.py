...
def lama_inpaint(src_list, mask_list, output_path:Path):
    ...
    # 1. Determine the best device (including Mac MPS)
    if torch.cuda.is_available():
        device_str = "cuda"
    elif torch.backends.mps.is_available():
        device_str = "mps"
    ...
    print(f"🎨 Inpainting using Device: {device_str}")

    # 2. Tell SimpleLama explicitly to use that device
    simple_lama = SimpleLama(device=torch.device(device_str))
